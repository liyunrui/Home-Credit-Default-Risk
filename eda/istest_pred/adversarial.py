...
def predict_istest(df, num_folds, stratified = False):
    df['istest'] = df['TARGET'].isnull()
    if stratified:
    ...
    feats = [f for f in df.columns if f not in ['TARGET','SK_ID_CURR','SK_ID_BUREAU','SK_ID_PREV','index','istest']]

    for n_fold, (train_idx, valid_idx) in enumerate(folds.split(df[feats], df['istest'])):
        train_x, train_y = df[feats].iloc[train_idx], df['istest'].iloc[train_idx]
        valid_x, valid_y = df[feats].iloc[valid_idx], df['istest'].iloc[valid_idx]
    ...
    over_folds_val_auc = roc_auc_score(df['istest'], oof_preds)
    print('---------------------------------------\nOver-folds val AUC score %.6f\n---------------------------------------' % over_folds_val_auc)
    # Write submission file and plot feature importance
    df.loc[:,'istest'] = oof_preds
    df[['SK_ID_CURR', 'istest']].to_csv(pred_file_name, index= False)
    real_train_df = df[df['TARGET'].notnull()]['istest'].hist()
    plt.savefig('adversarial-istest_prediction-distribution.png')
...
def main():
    ...
    with timer("Process credit card balance"):
        ...
        del cc
        gc.collect()
    with timer("Run Adversarial test with kfold"):
        # print(df.shape)
        # df.drop(feature_graveyard, axis=1, inplace=True, errors='ignore')
        # gc.collect()
        print(df.shape)

        feature_importance_df, over_folds_val_auc = predict_istest(df, num_folds= NUM_FOLDS, stratified= STRATIFIED)
        ...
        feature_importance_df_mean.to_csv("feature_importance-istest_pred.csv", index = True)

if __name__ == "__main__":
    pred_file_name = "istest_pred.csv"
