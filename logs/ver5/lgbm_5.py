...
features_dropped = [
    ...
    'FLAG_EMAIL', # fe2(rem)
    # rm set 4 (imp=0 in at least 3 folds)
...
def application_train_test(nan_as_category = False):
    ...
    df['NEW_DOC_IND_AVG'] = df[docs].mean(axis=1) # rm1
    df['NEW_DOC_IND_STD'] = df[docs].std(axis=1) # rm2
    df['NEW_DOC_IND_KURT'] = df[docs].kurtosis(axis=1)
    df['NEW_LIVE_IND_SUM'] = df[live].sum(axis=1) # rm3
...
def previous_applications(nan_as_category = True):
    ...
    # prev.loc[prev['DAYS_LAST_DUE']==365243,'DAYS_LAST_DUE'] = prev['DAYS_LAST_DUE_1ST_VERSION'] # fe4
    ...
    prev['TIME_DECAYED_HUMAN_EVAL'] = prev['HUMAN_EVAL'] / prev['DAYS_DECISION']**2 # fe2
    # prev['TIME_DECAYED_HUMAN_EVAL'] = prev['HUMAN_EVAL'] / (prev['DAYS_DECISION']**3) # fe5
    # prev['PERMISSION_RATE'] = prev['AMT_ANNUITY'] / prev['AMT_APPLICATION'] # fe1
    # prev['REAL_TERMS'] = prev['AMT_CREDIT'] / prev['AMT_ANNUITY'] # fe3
    # prev['INTEREST'] = prev['AMT_ANNUITY'] * prev['CNT_PAYMENT'] / prev['AMT_CREDIT'] - 1 # fe7
    ...
    num_aggregations = {
        ...
        # 'PERMISSION_RATE': ['min', 'max', 'mean', 'sum'], # fe1
        # 'REAL_TERMS': ['min', 'max', 'mean', 'sum'], # fe3
        # 'INTEREST': ['min', 'max', 'mean'], # fe7
    }
    # # rm6
    # num_aggregations = {
    #     'AMT_ANNUITY': ['max', 'mean'],
...
def installments_payments(nan_as_category = True):
    ...
    ins['DBD'] = ins['DPD'].apply(lambda x: x if x < 0 else 0)
