...
feature_graveyard = [
    ...
    'PREV_CODE_REJECT_REASON_HC_MEAN', 'NEW_RATIO_BURO_DAYS_CREDIT_UPDATE_MEAN', 'NAME_HOUSING_TYPE_Rented apartment', # fe22 revive 'NAME_HOUSING_TYPE_Rented apartment'
...
def application_train_test(nan_as_category = False):
    ...
    # df.replace({'NAME_EDUCATION_TYPE': {'Academic degree':4, 'Higher education':3, 'Incomplete higher':2.5, 'Secondary / secondary special':2, 'Lower secondary':1}}, 2, inplace= True) # fe17
    ...
    # df['NEW_DOC_IND_MED'] = df[docs].median(axis=1) # fe24
    ...
    # df['INCOME_GROW_RATE'] = df['AMT_INCOME_TOTAL'] / df['DAYS_EMPLOYED'] # fe11
    # df['CAR_VALUE'] = 100 / (2 ** df['OWN_CAR_AGE']) # fe13
    # df['COME_ON_WEEKDAY'] = 100 / (2 ** df['OWN_CAR_AGE']) # fe14
    # df['LIVING_TO_NONLIVING_RATIO'] = df['LIVINGAPARTMENTS_AVG'] / df['NONLIVINGAPARTMENTS_AVG'] # fe15
    # df['LIVING_TO_NONLIVING_RATIO'] = df['LIVINGAREA_AVG'] / df['NONLIVINGAREA_AVG'] # fe16
    # df['LIVING_TO_NONLIVING_RATIO'] = df['LIVINGAREA_MODE'] / df['NONLIVINGAREA_MODE'] # fe18
    # df['LIVING_TO_NONLIVING_RATIO'] = df['LIVINGAREA_MEDI'] / df['NONLIVINGAREA_MEDI'] # fe19
    # df['LIVING_TO_NONLIVING_RATIO'] = (df['LIVINGAREA_MEDI']+df['LIVINGAREA_MODE']+df['LIVINGAREA_AVG']) / (df['NONLIVINGAREA_MEDI']+df['NONLIVINGAREA_MODE']+df['NONLIVINGAREA_AVG']) # fe20
    # df['TERMS'] = df['AMT_CREDIT'] / df['AMT_ANNUITY'] # fe21
    ...
    # df['NAME_FAMILY_STATUS_Married'] = df['NAME_FAMILY_STATUS_Married'] + df['NAME_FAMILY_STATUS_Civil marriage'] # fe23
...
def pos_cash(nan_as_category = True):
    ...
    # pos['TIME_DECAYED_UNPAYED_RATIO'] = (1000 / pos['MONTHS_BALANCE']**3) * pos['CNT_INSTALMENT_FUTURE'] / pos['CNT_INSTALMENT']  # fe12
...
def installments_payments(nan_as_category = True):
    ...
    # ins['DPD'] = ins['DPD'] * ins['NUM_INSTALMENT_NUMBER'] # fe26

    # Features: Perform aggregations
    aggregations = {
        'NUM_INSTALMENT_VERSION': ['nunique'],
        # 'NUM_INSTALMENT_VERSION': ['var'], # fe25
        'DPD': ['max', 'mean', 'sum'],
        'DBD': ['max', 'mean', 'sum'],
