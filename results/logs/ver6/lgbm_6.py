...
features_dropped = [
    ...
    'REFUSED_RATE_DOWN_PAYMENT_MIN', 'REG_CITY_NOT_WORK_CITY', 'REG_REGION_NOT_LIVE_REGION', 'REG_REGION_NOT_WORK_REGION', 'FLAG_EMAIL',
...
def application_train_test(nan_as_category = False):
    ...
    # df.replace({'NAME_EDUCATION_TYPE': {'Academic degree':4, 'Higher education':3, 'Incomplete higher':2.5, 'Secondary / secondary special':2, 'Lower secondary':1}}, 2, inplace= True) # fe1
    ...
    df['NEW_DOC_IND_KURT'] = df[docs].kurtosis(axis=1) # rm5
    df['NEW_LIVE_IND_SUM'] = df[live].sum(axis=1) # rm1 good
    df['NEW_LIVE_IND_STD'] = df[live].std(axis=1)
    df['NEW_LIVE_IND_KURT'] = df[live].kurtosis(axis=1) # rm6 good
    ...
    # df['CREDIT_TERM'] = df['AMT_CREDIT'] / df['AMT_ANNUITY'] # fe8
    # df['CAPITAL_CREDIT_PREC'] = (df['FLAG_OWN_CAR'].apply(lambda x: 521500 if x == 1 else 0) + df['FLAG_OWN_REALTY'].apply(lambda x: 5291000 if x == 1 else 0)) / df['AMT_CREDIT'] # fe9
    # df['ANNUAL_BURDEN'] = df['CNT_FAM_MEMBERS']*10000*12 + df['AMT_ANNUITY'] # fe10 good
...
def previous_applications(nan_as_category = True):
    ...
    # prev.loc[prev['DAYS_LAST_DUE']==365243,'DAYS_LAST_DUE'] = prev['DAYS_LAST_DUE_1ST_VERSION'] # fe5
    # prev['TIME_LEFT_BEFORE_LAST_DUE'] = prev['DAYS_LAST_DUE_1ST_VERSION'] - prev['DAYS_LAST_DUE'] # fe6 slightly good
    # prev['TIME_LEFT_BEFORE_LAST_DUE'].fillna(prev['TIME_LEFT_BEFORE_LAST_DUE'].median()) # fe6 slightly good
    ...
    # prev['PERMISSION_RATE'] = prev['AMT_ANNUITY'] / prev['AMT_APPLICATION'] # fe2
    # prev['REAL_TERMS'] = prev['AMT_CREDIT'] / prev['AMT_ANNUITY'] # fe4 good
