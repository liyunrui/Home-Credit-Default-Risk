...
def application_train_test(nan_as_category = False):
    ...
    # df['NEW_CREDIT_TO_INCOME_RATIO'] = df['AMT_CREDIT'] / df['AMT_INCOME_TOTAL'] # duplicate
    ...
    contact = [_f for _f in df.columns if ('FLAG_' in _f) & ('FLAG_DOC' not in _f) & ('FLAG_OWN_' not in _f) & ('_FLAG_' not in _f)] # remove _FLAG_
...
def installments_payments(nan_as_category = True):
    ...
    ins['PAYMENT_PERC'] = ins['AMT_PAYMENT'] / ins['AMT_INSTALMENT'] # fe7(rem) slightly worse
    ins['PAYMENT_DIFF'] = ins['AMT_INSTALMENT'] - ins['AMT_PAYMENT'] # fe10(rem) worse
    # Days past due and days before due (no negative values)
    ins['DPD'] = ins['DAYS_ENTRY_PAYMENT'] - ins['DAYS_INSTALMENT'] # fe11 DAYS_DELAY, DEFAULT or not 
    ins['DBD'] = ins['DAYS_INSTALMENT'] - ins['DAYS_ENTRY_PAYMENT'] # fe2(rem) good to keep
    ins['DPD'] = ins['DPD'].apply(lambda x: x if x > 0 else 0) # fe2(rem) good to keep # fe4(rem) # fe6(+1 seperate before/past) # fe9(default or not)
    ins['DBD'] = ins['DBD'].apply(lambda x: x if x > 0 else 0) # fe2(rem) good to keep # fe4(rem) # fe5(count "on time") # fe6(+1 seperate before/past) # fe9(default or not) #fe12(rm DBD keep DPD)
    # ins['ON_TIME'] = ins['DPD'].apply(lambda x: 1 if x == 0 else 0) # fe8
    # ins['DPD_RATIO'] = ins['DAYS_INSTALMENT'] / ins['DAYS_ENTRY_PAYMENT'] # fe3(add) bad when duplicate

    # Features: Perform aggregations
    aggregations = {
        'NUM_INSTALMENT_VERSION': ['nunique'],
        'DPD': ['max', 'mean', 'sum'],
        # 'DPD_RATIO': ['max', 'mean', 'sum'], # fe3 bad when duplicate
        'DBD': ['max', 'mean', 'sum'], # fe2
        # 'ON_TIME': ['max', 'mean', 'sum'], # fe8
        'PAYMENT_PERC': ['max', 'mean', 'sum', 'var'], # fe7 good
        'PAYMENT_DIFF': ['max', 'mean', 'sum', 'var'], # fe10
