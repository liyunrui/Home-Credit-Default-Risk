...
def application_train_test(nan_as_category = False):
    ...
    # df.replace({'NAME_EDUCATION_TYPE': {'Academic degree':4, 'Higher education':3, 'Incomplete higher':2.5, 'Secondary / secondary special':2, 'Lower secondary':1}}, 2, inplace= True) # fe5
    ...
    # df['CAPITAL_CREDIT_PREC'] = (df['FLAG_OWN_CAR'].apply(lambda x: 521500 if x == 1 else 0) + df['FLAG_OWN_REALTY'].apply(lambda x: 5291000 if x == 1 else 0)) / df['AMT_CREDIT'] # fe6
    # df['ANNUAL_BURDEN'] = df['CNT_FAM_MEMBERS']*10000*12 + df['AMT_ANNUITY'] # fe7

    # replace NEW_SCORES_STD
    # df['EXT_SOURCE_1_VAR'] = (df['EXT_SOURCE_1'] - df['NEW_EXT_SOURCES_MEAN'])**2 # fe2
    # df['EXT_SOURCE_1_VAR'] = df['EXT_SOURCE_1_VAR'].fillna(df['EXT_SOURCE_1_VAR'].median()) # fe3
    # df['EXT_SOURCE_2_VAR'] = (df['EXT_SOURCE_2'] - df['NEW_EXT_SOURCES_MEAN'])**2 # fe2
    # df['EXT_SOURCE_2_VAR'] = df['EXT_SOURCE_1_VAR'].fillna(df['EXT_SOURCE_1_VAR'].median()) # fe3
    # df['EXT_SOURCE_3_VAR'] = (df['EXT_SOURCE_3'] - df['NEW_EXT_SOURCES_MEAN'])**2 # fe2
    # df['EXT_SOURCE_3_VAR'] = df['EXT_SOURCE_1_VAR'].fillna(df['EXT_SOURCE_1_VAR'].median()) # fe3
    ...
    # df['DAYS_EMPLOYED_PERC'] = df['DAYS_EMPLOYED'] / df['DAYS_BIRTH'] # duplicate
    # df['INCOME_CREDIT_PERC'] = df['AMT_INCOME_TOTAL'] / df['AMT_CREDIT'] # duplicate
    df['INCOME_PER_PERSON'] = df['AMT_INCOME_TOTAL'] / df['CNT_FAM_MEMBERS'] # fe1
    # df['ANNUITY_INCOME_PERC'] = df['AMT_ANNUITY'] / df['AMT_INCOME_TOTAL'] # duplicate
    # df['PAYMENT_RATE'] = df['AMT_ANNUITY'] / df['AMT_CREDIT'] # duplicate # duplicate
...
def previous_applications(nan_as_category = True):
    ...
    # prev['PAYMENT_RATE'] = prev['AMT_ANNUITY'] / prev['AMT_CREDIT'] # fe8
    ...
    num_aggregations = {
        ...
        # 'PAYMENT_RATE': ['min', 'max', 'mean', 'var'], # fe8
...
def installments_payments(nan_as_category = True):
    ...
    # ins['DPD'] = ins['DPD'].apply(lambda x: x+10 if x > 0 else 0) # fe12
    # ins['DBD'] = ins['DBD'].apply(lambda x: x+10 if x > 0 else 0) # fe12

    # ins['DPD'] = ins['DPD'].apply(lambda x: 1 if x > 0 else 0) # fe14
    # ins['DBD'] = ins['DBD'].apply(lambda x: 1 if x >= 0 else 0) # fe14
